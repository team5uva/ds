...
void Thread::processClientMulticast(Message* msg) {
	if (msg->type == CLIENT_ADDED || msg->type == TEXT_FROM_SERVER || msg->type == ACTION_FROM_SERVER || NAMECHANGE_FROM_SERVER)
...
void Thread::runServer(Server* s) {
	while (!m_stoprequested) {
			...
			while (true) {
				...
				if (receivedMessage != NULL) {
					receivedMessage->parseData();
					cout << "From Child Server Message type: " << receivedMessage->type << "\n";
					//processClientMessage(c, receivedMessage);
					dontSleep = true;
				}
